...
#define APP_SCPPS_SCAN_REFRESH_TO     1400     // 14s
...
void app_task_msg_hdl(ke_msg_id_t const msgid, void const *param)
{
    switch(msgid)
    {
        ...
        case SCPPS_DISABLE_IND:
            ke_timer_clear(APP_SCPPS_SCAN_REFRESH_TIMER, TASK_APP);
            break;

        case SCPPS_SCAN_REFRESH_NTF_CFG_IND:
            if (((struct scpps_scan_refresh_ntf_cfg_ind *)param)->scan_refresh_ntf_en == PRF_CLI_START_NTF)
            {
