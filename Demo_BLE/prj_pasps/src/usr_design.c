...
#define APP_PASPS_UPDATE_ALERT_STATUS          1000    // 10s
#define APP_PASPS_UPDATE_RINGER_SET            2000    // 20s
/*
...
void app_task_msg_hdl(ke_msg_id_t const msgid, void const *param)
{
    switch(msgid)
    {
        ...
        case PASPS_DISABLE_IND:
            ke_timer_clear(APP_PASPS_UPDATE_ALERT_STATUS_TIMER, TASK_APP);
            ke_timer_clear(APP_PASPS_UPDATE_RINGER_TIMER, TASK_APP);
            break;

        case PASPS_WRITTEN_CHAR_VAL_IND:
        {
            struct pasps_written_char_val_ind *cfg = (struct pasps_written_char_val_ind *)param;
            
            if (cfg->att_code == PASPS_ALERT_STATUS_NTF_CFG)
            {
                if (app_pasps_env->app_cfg & PASPS_FLAG_ALERT_STATUS_CFG)
                {
                    app_pasps_env->ntf_sending |= PASPS_FLAG_ALERT_STATUS_CFG;
                    // Send alert status period for demo
                    ke_timer_set(APP_PASPS_UPDATE_ALERT_STATUS_TIMER, TASK_APP, APP_PASPS_UPDATE_ALERT_STATUS);
                }
                else
                {
                    ke_timer_clear(APP_PASPS_UPDATE_ALERT_STATUS_TIMER, TASK_APP);
            ...
            else if (cfg->att_code == PASPS_RINGER_SETTING_NTF_CFG)
            {
                if (app_pasps_env->app_cfg & PASPS_FLAG_RINGER_SETTING_CFG)
                {
                    app_pasps_env->ntf_sending |= PASPS_FLAG_RINGER_SETTING_CFG;
                    // Send ringer setting status period for demo
                    ke_timer_set(APP_PASPS_UPDATE_RINGER_TIMER, TASK_APP, APP_PASPS_UPDATE_RINGER_SET);
                }
                else
                {
                    ke_timer_clear(APP_PASPS_UPDATE_RINGER_TIMER, TASK_APP);
            ...
            else if (cfg->att_code == PASPS_RINGER_CTNL_PT_CHAR_VAL)
            {
                switch (cfg->value.ringer_ctnl_pt)
                {
                    case PASP_SILENT_MODE:
                        app_pasps_update_char_val_req(app_pasps_env->conhdl,
...
/**
 ****************************************************************************************
 * @brief Handles the demo Phone Alert Status Service timer.
 *
 ...
 * @return If the message was consumed or not.
 * @description 
 *  Handles the demo PASS timer.
 ****************************************************************************************
 */
int app_pasps_update_alert_timer_handler(ke_msg_id_t const msgid,
                                      void *param,
                                      ke_task_id_t const dest_id,
                                      ke_task_id_t const src_id)
{
    //DEVELOPER NOTE: SET REAL VALUE OF USER APPLICATION IN THIS FUNCTION
    if ((app_pasps_env->ntf_sending & PASPS_FLAG_ALERT_STATUS_CFG) &&
 ...
 ****************************************************************************************
 */
int app_pasps_update_ringer_timer_handler(ke_msg_id_t const msgid,
                                      void *param,
                                      ke_task_id_t const dest_id,
                                      ke_task_id_t const src_id)
{
    //DEVELOPER NOTE: SET REAL VALUE OF USER APPLICATION IN THIS FUNCTION
    if ((app_pasps_env->ntf_sending & PASPS_FLAG_RINGER_SETTING_CFG) &&
